...
static _shader_routine shader_routine[SHADER_TOTAL] = {
	...
	/*****************************************************************************************/
	{
			...
			"	vec3 D = texture2D(s_texture_scr, d11-dx   ).xyz;\n"
			"	vec3 E = texture2D(s_texture_scr, d11      ).xyz;\n"
			"	vec3 F = texture2D(s_texture_scr, d11+dx   ).xyz;\n"
			"	vec3 G = texture2D(s_texture_scr, d11-dx+dy).xyz;\n"
			"	vec3 H = texture2D(s_texture_scr, d11+dy   ).xyz;\n"
			"	vec3 I = texture2D(s_texture_scr, d11+dx+dy).xyz;\n"
