...
#define m45chr1k(vl)\
	bank = (m45.reg[0] | ((m45.reg[2] << 4) & 0x0F00))\
		...
		m45.reg[2] ? 0 : vl)
#define m45prg8k(vl) value = m45.reg[1] | (vl & ((m45.reg[3] & 0x3F) ^ 0x3F))
#define m45chr1kupdate()\
{\
	BYTE i;\
	for (i = 0; i < 8; i++) {\
		WORD bank;\
		m45chr1k(m45.chrmap[i]);\
		_controlBank(bank, chrRom1kMax)\
		chr.bank1k[i] = &chr.data[bank << 10];\
	}\
}
#define m45prg8kupdate()\
{\
	BYTE i;\
	for (i = 0; i < 4; i++) {\
		m45prg8k(m45.prgmap[i]);\
		controlBank(prgRom8kMax)\
		mapPrgRom8k(1, i, value);\
	}\
	mapPrgRom8kUpdate();\
}
#define m45swapChrBank1k(src, dst)\
{\
	BYTE *chrBank1k = chr.bank1k[src];\
	chr.bank1k[src] = chr.bank1k[dst];\
	chr.bank1k[dst] = chrBank1k;\
	WORD map = m45.chrmap[src];\
	m45.chrmap[src] = m45.chrmap[dst];\
	m45.chrmap[dst] = map;\
}
#define m45_8000()\
{\
	...
	if (mmc3.chrRomCfg != chrRomCfgOld) {\
		m45swapChrBank1k(0, 4)\
		m45swapChrBank1k(1, 5)\
		m45swapChrBank1k(2, 6)\
		m45swapChrBank1k(3, 7)\
	}\
...
{\
	...
	switch (mmc3.bankToUpdate) {\
		case 0:\
			m45.chrmap[mmc3.chrRomCfg] = value;\
			m45.chrmap[mmc3.chrRomCfg | 0x01] = value + 1;\
			m45chr1k(value);\
			bank &= 0xFFE;\
			_controlBank(bank, chrRom1kMax)\
			...
			return;\
		case 1:\
			m45.chrmap[mmc3.chrRomCfg | 0x02] = value;\
			m45.chrmap[mmc3.chrRomCfg | 0x03] = value + 1;\
			m45chr1k(value);\
			bank &= 0xFFE;\
			_controlBank(bank, chrRom1kMax)\
...
void mapInit_45(void) {
	...
	mapper.intStruct[0] = (BYTE *) &m45;
	mapper.intStructSize[0] = sizeof(m45);
...

	{
		...
		for (i = 0; i < 8; i++) {
			if (i < 4) {
				m45.prgmap[i] = mapper.romMapTo[i];
			}
			m45.chrmap[i] = i;
		}
...
BYTE extcl_save_mapper_45(BYTE mode, BYTE slot, FILE *fp) {
	savestateEle(mode, slot, m45.reg);
	savestateEle(mode, slot, m45.index);
	savestateEle(mode, slot, m45.prgmap);
	savestateEle(mode, slot, m45.chrmap);
