...
typedef struct {
	...
	float baseHz;          /* il clock di base (Hz) */
	BYTE baseDivide;       /* il divisore del clock di base */
	float masterHz;        /* master clock (Hz) = ((baseClock / baseClockDivide) * 6) */
	...
	float ppuHz;           /* PPU clock (masterHz / ppuDivide) */
	...
	float cpuHz;           /* CPU clock (masterHz / cpuDivide) */
