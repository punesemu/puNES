...
BYTE cpu_rd_mem(WORD address, BYTE made_tick) {
	...
	if (address <= 0x4017) {
		...
		if (address < 0x2000) {
			...
			/* leggo */
			cpu.openbus.actual = ram_rd(address);
	...
	if (address < 0x8000) {
		...
		if (extcl_cpu_rd_mem) {
			cpu.openbus.actual = extcl_cpu_rd_mem(address, pippo/*cpu.openbus.actual*/);
		}
...
INLINE static void nsf_rd_mem(WORD address, BYTE made_tick) {
	...
	if (address < 0x2000) {
		...
		cpu.openbus.actual = ram_rd(address & 0x7FF);
		return;
	}
	// NSF Player Routine
