...
#define ABS(opTy, cmd)\
{\
	WORD adr0 = lend_word(cpu.PC.w, FALSE, TRUE);\
	cpu.PC.w += 2;\
	cmd\
}
#define ABW(opTy, cmd)\
{\
	WORD adr0 = lend_word(cpu.PC.w, FALSE, FALSE);\
	_DMC\
	cpu.PC.w += 2;\
	cmd\
}
...
{\
	WORD adr2 = lend_word(cpu.PC.w, FALSE, TRUE);\
	WORD adr0 = adr2 + (reg);\
...
{\
	WORD adr0 = lend_word(cpu.PC.w, FALSE, TRUE);\
	cpu.PC.w = lend_word(adr0, TRUE, TRUE);\
}
#define IDX(opTy, cmd)\
{\
	WORD adr1 = _RDP;\
...
#define BRC(flag, condition) \
	BYTE offset = _RDP;\
	WORD adr0 = cpu.PC.w + (SBYTE)offset;\
	if ((!flag) != condition) {\
		...
		BYTE cross = !((adr0 & 0xFF00) == (cpu.PC.w & 0xFF00));\
		if (!cross) {\
...
//  varie ed eventuali
// ----------------------------------------------------------------------
#define _ADC _RSZ(_ADD(cpu.openbus), cpu.AR)
// NOTE : BCD Addiction
// 6502
// A Seq. 1
// C Seq. 1
...
#define _ADD(opr)\
	...
	if (info.decimal_mode && cpu.df) {\
		BYTE AL = (cpu.AR & 0x0F) + ((opr) & 0x0F) + cpu.cf;\
		if (AL >= 0x0A) { AL = ((AL + 0x06) & 0x0F) + 0x10; }\
		A = (cpu.AR & 0xF0) + ((opr) & 0xF0) + AL;\
		if (A >= 0xA0) { A += 0x60; }\
	} else { A = cpu.AR + (opr) + cpu.cf; }\
	cpu.cf = (A > 0xFF ? 1 : 0);\
	cpu.of = ((!((cpu.AR ^ (opr)) & 0x80) && ((cpu.AR ^ A) & 0x80)) ? 0x40 : 0);\
	cpu.AR = (BYTE)A;\
	}
...
#define _RDP _RDX(cpu.PC.w++, TRUE)
#define _RDD _RDX(cpu.PC.w, TRUE)
...
#define _RDZ(cmd, result)\
    _RDD;\
	_RSZ(cmd, result)
#define _SBC _RSZ(_SUB(cpu.openbus), cpu.AR)
...
#define _SUB(opr)\
	{\
	if (info.decimal_mode && cpu.df) {\
		SWORD A = 0; SWORD AL = (cpu.AR & 0x0F) - ((opr) & 0x0F) + cpu.cf - 1;\
		AL = (cpu.AR & 0x0F) - ((opr) & 0x0F) + cpu.cf - 1;\
...
#define _IRQ(flags)\
	...
	if (flagNMI) {\
		...
		cpu.PC.w = lend_word(INT_NMI, FALSE, TRUE);\
	} else {\
		cpu.PC.w = lend_word(INT_IRQ, FALSE, TRUE);\
		if (nmi.high) {\
			nmi.delay = TRUE;\
		}\
	}
...
void cpu_exe_op(void) {
	...
	case 0x0A: IMP(RD_OP, _RDZ(_BSH(cpu.AR, 0x80, <<=), cpu.AR)) break;  // ASL [AR]
	...
	case 0x18: IMP(RD_OP, _RDD; cpu.cf = FALSE;) break;                  // CLC [C -> 0]
	case 0xD8: IMP(RD_OP, _RDD; cpu.df = FALSE;) break;                  // CLD [D -> 0]
	case 0x58: IMP(RD_OP, _RDD; cpu.im = FALSE;) break;                  // CLI [I -> 0]
	case 0xB8: IMP(RD_OP, _RDD; cpu.of = FALSE;) break;                  // CLV [O -> 0]
	...
	case 0xCA: IMP(RD_OP, _RDZ(cpu.XR--;, cpu.XR)) break;                // DEX [XR]

	case 0x88: IMP(RD_OP, _RDZ(cpu.YR--;, cpu.YR)) break;                // DEY [YR]
	...
	case 0xE8: IMP(RD_OP, _RDZ(cpu.XR++;, cpu.XR)) break;                // INX [XR]

	case 0xC8: IMP(RD_OP, _RDZ(cpu.YR++;, cpu.YR)) break;                // INY [YR]
	...
	case 0x4A: IMP(RD_OP, _RDZ(_BSH(cpu.AR, 0x01, >>=), cpu.AR)) break;  // LSR [AR]
	...
	case 0x2A: IMP(RD_OP, _RDZ(_ROL(cpu.AR, 0x80, <<=), cpu.AR)) break;  // ROL [AR]
	...
	case 0x6A: IMP(RD_OP, _RDZ(_ROR(cpu.AR, 0x01, >>=), cpu.AR)) break;  // ROR [AR]
	...
	case 0x38: IMP(RD_OP, _RDD; cpu.cf = 0x01;) break;                   // SEC [C -> 1]
	case 0xF8: IMP(RD_OP, _RDD; cpu.df = 0x08;) break;                   // SED [D -> 1]
	...
	case 0xAA: IMP(RD_OP, _RDZ(cpu.XR = cpu.AR;, cpu.XR)) break;         // TAX
	case 0xA8: IMP(RD_OP, _RDZ(cpu.YR = cpu.AR;, cpu.YR)) break;         // TAY
	case 0xBA: IMP(RD_OP, _RDZ(cpu.XR = cpu.SP;, cpu.XR)) break;         // TSX
	case 0x8A: IMP(RD_OP, _RDZ(cpu.AR = cpu.XR;, cpu.AR)) break;         // TXA
	case 0x9A: IMP(RD_OP, _RDD; cpu.SP = cpu.XR;) break;                 // TXS
	case 0x98: IMP(RD_OP, _RDZ(cpu.AR = cpu.YR;, cpu.AR)) break;         // TYA
	...
	case 0x89:                                                           // DOP #IMM
	case 0XC2:                                                           // DOP #IMM
	case 0XE2: cpu.PC.w++; break;                                        // DOP #IMM
	case 0x04:                                                           // DOP $ZPG
	case 0x44:                                                           // DOP $ZPG
	...
	default:
		if (!info.no_rom && !info.first_illegal_opcode) {
			log_warning(uL("cpu;alert PC = 0x%04X, CODEOP = 0x%02X"), (cpu.PC.w - 1), cpu.opcode);
...
void cpu_initial_cycles(void) {
	cpu_rd_mem(cpu.PC.w, TRUE);
	cpu_rd_mem(cpu.PC.w, TRUE);
	cpu_rd_mem(cpu.PC.w, TRUE);
	cpu_rd_mem(cpu.SP-- | STACK, TRUE);
	cpu_rd_mem(cpu.SP-- | STACK, TRUE);
	cpu_rd_mem(cpu.SP-- | STACK, TRUE);
	cpu.PC.b[0] = cpu_rd_mem(INT_RESET, TRUE);
	cpu.PC.b[1] = cpu_rd_mem(INT_RESET + 1, TRUE);
	cpu.cycles = 0;
	cpu.opcode_cycle = 0;
	cpu.double_wr = 0;
	cpu.double_rd = 0;
