...
enum nsf_text_scroll {
	NSF_TEXT_SCROLL_INC_NONE,
	NSF_TEXT_SCROLL_INC_START,
	NSF_TEXT_SCROLL_INC_COUNT
};
...
static char *nsf_text_scroll_tick(_nsf_text_scroll *scroll);
...
BYTE nsf_load_rom(void) {
	...
	if ((rom.data[rom.position++] == 'N') &&
...

		{
			...
			memset(nsf.info.auth, 0x00, (32 + 1) * 4);
...
void nsf_after_load_rom(void) {
	...
	memset(nsf.scroll_info_nsf.buffer, 0x00, sizeof(nsf.scroll_info_nsf.buffer));
	snprintf(nsf.scroll_info_nsf.buffer, sizeof(nsf.scroll_info_nsf.buffer),
		"[cyan]%s[normal] - [yellow]%s[normal] - %s",
		nsf.info.name, nsf.info.artist, nsf.info.copyright);

	nsf.scroll_title_song.rows = NSF_GUI_INFO_SONG_TITLE_ROW;
	nsf.scroll_title_song.reload = 250;
	...
	if (nsf.timers.update_only_diff == FALSE) {
		...
		if ((nsf.current_song.time > 0) && (nsf.current_song.fade > 0)) {
			if (nsf.timers.song > (nsf.current_song.time - nsf.current_song.fade)) {
				...
				if (cfg->nsf_player_nsfe_fadeout == TRUE) {
					double divider;
			...
			if ((silence == TRUE) && (nsf.songs.started == TRUE)) {
...
void nsf_reset_song_title(void) {
	if (nsf.info_song) {
		...
		memset(nsf.scroll_title_song.buffer, 0x00, sizeof(nsf.scroll_title_song.buffer));
		snprintf(nsf.scroll_title_song.buffer, sizeof(nsf.scroll_title_song.buffer), "%s",
...
#define nsf_c_y2() (nsf.effect_coords.y2)
	...
	if (((x_mouse >= nsf_c_x1()) && (x_mouse <= nsf_c_x2())) &&
		((y_mouse >= nsf_c_y1()) && (y_mouse <= nsf_c_y2()))) {
...
static void nsf_effect_raw(BYTE solid) {
	int x, y, count, len, y_last = nsf.effect_coords.y_center;
	SWORD *buffer = NULL;
...
static void nsf_effect_hanning_window(BYTE solid) {
	...
	for (x = nsf.effect_coords.x1, count = 0; x <= nsf.effect_coords.x2; x++, count++) {
		...
		if (!buffer || !len) {
			...
			y = nsf.effect_coords.y_center + (((multiplier *
				((buffer[(count * len) / nsf.effect_coords.w]))) / (16384 / nsf.effect_coords.h)) *
				(cfg->apu.channel[APU_MASTER] * cfg->apu.volume[APU_MASTER]));
		}
...
static void nsf_effect_bars(void) {
	...
	if (!buffer || !len) {
		...
		for (count = 0; count < len; count++) {
			fft_in[count].r = 0.5f * (1.0f - cos((2.0f * M_PI * count) / (len - 1))) *
				((float) buffer[count] / 16384.0f);
			fft_in[count].i = 0;
		}
			...
			for (count = 0; count <= (len / 2); count++) {
				...
				if ((count > 0) && (count < len / 2)) {
					magnitude = sqrt(fft_out[count].r * fft_out[count].r +
						fft_out[count].i * fft_out[count].i);
				} else {
					magnitude = sqrt(fft_out[count].r * fft_out[count].r);
				}
	...
	for (count = 0; count < bars; count++) {
		int a, b, x1, x2;
		float value = (bar[count] * 16383.0f) / (16384.0f / (nsf.effect_bars_coords.h - 5));
		BYTE y = (nsf.effect_bars_coords.y2 - 5) -
			(value * (cfg->apu.channel[APU_MASTER] * cfg->apu.volume[APU_MASTER]));

		x1 = (float) (nsf.effect_bars_coords.x1 + ((nsf.effect_bars_coords.w / bars) * count));
		x2 = (float) (x1 + (nsf.effect_bars_coords.w / bars));
...
static void nsf_draw_controls(void) {
	...
	// titolo nsf - artista - copyright
	{
		...
		dos_text(0, dospf(3), nsf_text_scroll_tick(&nsf.scroll_info_nsf));
	}
...
#undef nsf_dc_box_ctrl
	...
	// canzoni
	{
		...
		if (nsf.info_song) {
			...
			if (nsf.playlist.count > 0) {
				if (cfg->nsf_player_nsfe_playlist == TRUE) {
					sprintf(buff, "[bck][gray][normal]Plst [gray]%s",
						nsf_print_number(nsf.playlist.index + 1, 3, DOS_CYAN));
	...
	if (nsf.state) {
		...
		if (nsf.info_song) {
			if (nsf.draw_mask == TRUE) {
				dos_vline(x + (w / 2), y + ((h / NSF_GUI_INFO_SONG_LINES) * 1) + 1,
					(h / NSF_GUI_INFO_SONG_LINES) - 1, 0x002D);
			}
			...
			if (nsf.current_song.time > 0) {
				dos_box(x + (w / 2) + 1, y + ((h / NSF_GUI_INFO_SONG_LINES) * 1) + 1, (w / 2) - 2,
					(dospf(1) * 2) - 1, doscolor(DOS_GRAY),
					doscolor(DOS_GRAY), doscolor(DOS_GRAY));
			} else {
				dos_box(x + (w / 2) + 1, y + ((h / NSF_GUI_INFO_SONG_LINES) * 1) + 1, (w / 2) - 2,
					(dospf(1) * 2) - 1, doscolor(DOS_BLACK),
					doscolor(DOS_BLACK), doscolor(DOS_BLACK));
				dos_vline(x + w - 2,   y + ((h / NSF_GUI_INFO_SONG_LINES) * 1) + 1,
					(h / NSF_GUI_INFO_SONG_LINES) - 1, 0x002D);
			}
			...
			if (nsf.current_song.time > 0) {
				if (nsf.options.visual_duration == TRUE) {
			...
			if (nsf.options.visual_duration == TRUE) {
				...
				dos_text(x + ((w - dospf(24)) / 2) + dospf(13),
					y + ((h / NSF_GUI_INFO_SONG_LINES) * 1) +
					(((h / NSF_GUI_INFO_SONG_LINES) - 8) / 2), "%s",
	...
	if (nsf.info_song) {
		if (nsf.draw_mask == TRUE) {
			dos_box(x + 2, y + ((h / NSF_GUI_INFO_SONG_LINES) * 2) + 1, w - 4,
				(dospf(1) * 2) - 3, doscolor(DOS_BLACK), doscolor(DOS_BLACK),
				doscolor(DOS_BLACK));
			dos_vline(x + 1, y + ((h / NSF_GUI_INFO_SONG_LINES) * 2) + 1,
				(h / NSF_GUI_INFO_SONG_LINES) - 1, 0x002D);
			dos_vline(x + w - 2, y + ((h / NSF_GUI_INFO_SONG_LINES) * 2) + 1,
				(h / NSF_GUI_INFO_SONG_LINES) - 1, 0x002D);
		}
		...
		if (nsf.current_song.track_label) {
			if (nsf.timers.song > 100) {
				if (strlen(nsf.current_song.track_label) > NSF_GUI_INFO_SONG_TITLE_ROW) {
					dos_text(x + ((w - dospf(nsf.scroll_title_song.rows)) / 2),
						y + ((h / NSF_GUI_INFO_SONG_LINES) * 2) + (((h / NSF_GUI_INFO_SONG_LINES) - 8) / 2),
	...
	// nsfe opzioni
	{
		...
		nsf_print_option(&nsf_options_data[NSF_OPTION_PLAYLIST], (info.format == NSFE_FORMAT),
			cfg->nsf_player_nsfe_playlist);
		nsf_print_option(&nsf_options_data[NSF_OPTION_FADEOUT], (info.format == NSFE_FORMAT),
			cfg->nsf_player_nsfe_fadeout);
	}
	...
	// indicatore effetto
	{
		...
		if (nsf.draw_mask == TRUE) {
			...
			dos_hline(w + (NSF_EFFECTS * (NSF_GUI_EFFECT_BOOKMARK_W + 2)) + 1, NSF_GUI_EFFECT_Y - 1,
				w, doscolor(DOS_BROWN));
		}
...
static char *nsf_print_number(unsigned int number, BYTE decimal, int color) {
	static char cbuff[30];
	BYTE ibuff[3], i, is_normal = FALSE;

	ibuff[0] =  number / 100;
	ibuff[1] = (number - (ibuff[0] * 100)) / 10;
	ibuff[2] =  number - (ibuff[0] * 100) - (ibuff[1] * 10);
...
static void nsf_print_option(_nsf_option_data *option, BYTE active, BYTE mode) {
	...
	if (option->x1 < 0) {
		if (option->direction) {
			option->x1 = option->x - NSF_GUI_OPTIONS_BOX_SPACE - NSF_GUI_OPTIONS_BOX_SIDE -
				dospf(dos_strlen(option->txt));
			option->x2 = option->x;
		} else {
			option->x1 = option->x;
			option->x2 = option->x + NSF_GUI_OPTIONS_BOX_SIDE + NSF_GUI_OPTIONS_BOX_SPACE +
				dospf(dos_strlen(option->txt));
		}
	}
...
static char *nsf_text_scroll_tick(_nsf_text_scroll *scroll) {
	if ((dos_strlen(scroll->buffer) > 0) && (scroll->timer <= 0)) {
		int i, count, now, tmp, tag, end = strlen(scroll->buffer);
		BYTE action = NSF_TEXT_SCROLL_INC_NONE;

		scroll->timer = scroll->reload;

		memset(scroll->string, 0x00, sizeof(scroll->string));
		strcpy(scroll->string, scroll->tags);

		i = strlen(scroll->string);
		now = scroll->index.start;

		for (count = 0; count < scroll->rows;) {
			if (scroll->index.count < scroll->rows) {
				action = NSF_TEXT_SCROLL_INC_COUNT;

				if (((scroll->rows - count) > scroll->index.count) && (now < end)) {
					scroll->string[i] = ' ';
					i++;
					count++;
					continue;
				...
				action = NSF_TEXT_SCROLL_INC_START;
			}
			...
			if (now >= end) {
				...
				if ((scroll->buffer[now] == '[') &&
					((tmp = dos_is_tag(scroll->buffer + now, &tag)) > 0)) {
					strncat(scroll->string, scroll->buffer + now, tmp);
					now += tmp;
					i += tmp;
					continue;
				}
				scroll->string[i] = scroll->buffer[now];
				now++;
				i++;
			}
		...
		switch (action) {
			case NSF_TEXT_SCROLL_INC_NONE:
				break;
			case NSF_TEXT_SCROLL_INC_COUNT:
				scroll->index.count++;
				break;
			case NSF_TEXT_SCROLL_INC_START:
				if (scroll->index.start < end) {
					while (scroll->buffer[scroll->index.start] == '[') {
						if ((tmp = dos_is_tag(scroll->buffer + scroll->index.start, &tag)) > 0) {
							if (tag <= DOS_BACKGROUND_COLOR) {
								strncat(scroll->tags, scroll->buffer + scroll->index.start, tmp);
							}
							scroll->index.start += tmp;
						}
					}
					scroll->index.start++;
				} else {
					scroll->index.count = 0;
					scroll->index.start = 0;
					memset(scroll->tags, 0x00, sizeof(scroll->tags));
				}
				break;
		}
	} else {
...
static void nsf_text_curtain(_nsf_text_curtain *curtain, BYTE mode) {
	if (mode == NSF_TEXT_CURTAIN_INIT) {
		...
		if (curtain->redraw.all == TRUE) {
			curtain->redraw.all = FALSE;

			dos_box(curtain->x, curtain->y, curtain->rows * dospf(1), dospf(1),
				doscolor(DOS_BLACK), doscolor(DOS_BLACK), doscolor(DOS_BLACK));

			_dos_text(curtain->x, curtain->y, -1, -1, curtain->redraw.bottom, -1,
				curtain->line[curtain->index].text);
		}
		if (curtain->timer <= 0) {
			if (curtain->pause == FALSE) {
				dos_hline(curtain->x, curtain->y + curtain->borders.bottom, curtain->rows * dospf(1),
					doscolor(DOS_BLACK));

				_dos_text(curtain->x, curtain->y, -1, -1, curtain->borders.bottom, -1,
					curtain->line[curtain->index].text);
...
static void nsf_text_curtain_add_line(_nsf_text_curtain *curtain, const char *fmt, ...) {
	...
	if ((line = realloc(curtain->line, (curtain->count + count) * sizeof(_nsf_text_curtain_line)))) {
		curtain->line = line;
...
static void nsf_reset_song_variables(void) {
	...
	if (nsf.info_song) {
		...
		nsf.curtain_title_song.y = y +
			((h / NSF_GUI_INFO_SONG_LINES) * 2) + (((h / NSF_GUI_INFO_SONG_LINES) - 8) / 2);
