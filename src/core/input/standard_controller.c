...
void input_wr_standard_controller(UNUSED(BYTE nidx), const BYTE *value, BYTE nport) {
	if ((r4016.value & 0x01) || ((*value) & 0x01)) {
		port[nport].index = 0;
	}
}
void input_rd_standard_controller(UNUSED(BYTE nidx), BYTE *value, BYTE nport, BYTE shift) {
