...
INLINE static void tmp_fix_237(BYTE max, BYTE index, const WORD *ds);
...
void map_init_237(void) {
	...
	if (info.reset == RESET) {
		...
		memset(&m237tmp, 0x00, sizeof(m237tmp));

		{
			static WORD ds[] = { 0x00 };

			tmp_fix_237(LENGTH(ds), 0, &ds[0]);
		}
	}
...
INLINE static void tmp_fix_237(BYTE max, BYTE index, const WORD *ds) {
	m237tmp.ds_used = TRUE;
	m237tmp.max = max;
	m237tmp.index = index;
	m237tmp.dipswitch = ds;
}
