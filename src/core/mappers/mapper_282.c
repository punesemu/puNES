...
void prg_swap_jyasic_282(WORD address, DBWORD value);
void chr_swap_jyasic_282(WORD address, DBWORD value);
void wram_swap_jyasic_282(WORD address, DBWORD value);
void mirroring_swap_jyasic_282(WORD address, DBWORD value);

INLINE static DBWORD prg_bank(DBWORD value);
INLINE static DBWORD chr_bank(DBWORD value);

void map_init_282(void) {
	...
	JYASIC_prg_swap = prg_swap_jyasic_282;
	JYASIC_chr_swap = chr_swap_jyasic_282;
	JYASIC_wram_swap = wram_swap_jyasic_282;
	JYASIC_mirroring_swap = mirroring_swap_jyasic_282;
}
...
INLINE static DBWORD prg_bank(DBWORD value) {
	return ((((jyasic.mode[3] << 4) & ~0x1F) | (value & 0x1F)));
}
INLINE static DBWORD chr_bank(DBWORD value) {
	WORD base = ((jyasic.mode[3] & 0x01) << 8) | ((jyasic.mode[3] & 0x18) << 6);
	WORD mask = 0xFF;
