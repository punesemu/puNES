...
void map_init_327(void) {
	...
	map_internal_struct_init((BYTE *)&m327, sizeof(m327));
...
void extcl_cpu_wr_mem_327(BYTE nidx, WORD address, BYTE value) {
	if ((address >= 0x6000) && (address <= 0x7FFF)) {
		if (!(m327.reg & 0x07) && memmap_adr_is_writable(nidx, MMCPU(address))) {
			m327.reg = address;
			MMC3_prg_fix();
			MMC3_chr_fix();
		}
