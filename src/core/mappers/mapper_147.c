...
void extcl_cpu_wr_mem_147(WORD address, BYTE value) {
	extcl_cpu_wr_mem_JV001(address, (((value & 0x03) << 6) | ((value & 0xFC) >> 2)));
}
BYTE extcl_cpu_rd_mem_147(WORD address, BYTE openbus) {
...
void prg_fix_jv001_147(void) {
	memmap_auto_32k(MMCPU(0x8000), (((jv001.output & 0x20) >> 4) | (jv001.output & 0x01)));
}
void chr_fix_jv001_147(void) {
	memmap_auto_8k(MMPPU(0x0000), ((jv001.output & 0x1E) >> 1));
}
