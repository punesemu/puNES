...
void wram_swap_jyasic_387(WORD address, DBWORD value);
void mirroring_swap_jyasic_387(WORD address, DBWORD value);

INLINE static DBWORD prg_bank(DBWORD value);
INLINE static DBWORD chr_bank(DBWORD value);

void map_init_387(void) {
	...
	JYASIC_prg_swap = prg_swap_jyasic_387;
	JYASIC_chr_swap = chr_swap_jyasic_387;
	JYASIC_wram_swap = wram_swap_jyasic_387;
	JYASIC_mirroring_swap = mirroring_swap_jyasic_387;
}
...
INLINE static DBWORD prg_bank(DBWORD value) {
	return ((((((jyasic.mode[3] & 0x02) << 3) | (jyasic.mode[3] & 0x08) << 2)) | (value & 0x0F)));
}
INLINE static DBWORD chr_bank(DBWORD value) {
