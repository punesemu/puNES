...
void wram_swap_jyasic_388(WORD address, DBWORD value);
void mirroring_swap_jyasic_388(WORD address, DBWORD value);

INLINE static DBWORD prg_bank(DBWORD value);
INLINE static DBWORD chr_bank(DBWORD value);

void map_init_388(void) {
	...
	JYASIC_mirroring_swap = mirroring_swap_jyasic_388;
}
...
INLINE static DBWORD prg_bank(DBWORD value) {
	return (((jyasic.mode[3] & 0x0C) << 3) | (value & 0x1F));
}
INLINE static DBWORD chr_bank(DBWORD value) {
	WORD base = ((jyasic.mode[3] & 0x01) << 8) | ((jyasic.mode[3] & 0x02) << 8);
	WORD mask = 0xFF;
