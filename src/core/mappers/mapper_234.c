...
void extcl_cpu_wr_mem_234(WORD address, BYTE value) {
...
INLINE static void prg_fix_234(void) {
	if (m234.reg[0] & 0x40) {
		memmap_auto_32k(MMCPU(0x8000), ((m234.reg[0] & 0x0E) | (m234.reg[1] & 0x01)));
	} else {
		memmap_auto_32k(MMCPU(0x8000), (m234.reg[0] & 0x0F));
	}
}
INLINE static void chr_fix_234(void) {
	if (m234.reg[0] & 0x40) {
		memmap_auto_8k(MMPPU(0x0000), (((m234.reg[0] & 0x0E) << 2) | ((m234.reg[1] & 0x70) >> 4)));
	} else {
		memmap_auto_8k(MMPPU(0x0000), (((m234.reg[0] & 0x0F) << 2) | ((m234.reg[1] & 0x30) >> 4)));
	}
}
INLINE static void mirroring_fix_234(void) {
	if (m234.reg[0] & 0x80) {
