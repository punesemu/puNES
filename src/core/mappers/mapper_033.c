...
void extcl_cpu_wr_mem_033(WORD address, BYTE value) {
...
INLINE static void prg_fix_033(void) {
	memmap_auto_8k(MMCPU(0x8000), m033.prg[0]);
	memmap_auto_8k(MMCPU(0xA000), m033.prg[1]);
	memmap_auto_16k(MMCPU(0xC000), 0xFF);
}
INLINE static void chr_fix_033(void) {
	memmap_auto_2k(MMPPU(0x0000), m033.chr[0]);
	memmap_auto_2k(MMPPU(0x0800), m033.chr[1]);
	memmap_auto_1k(MMPPU(0x1000), m033.chr[2]);
	memmap_auto_1k(MMPPU(0x1400), m033.chr[3]);
	memmap_auto_1k(MMPPU(0x1800), m033.chr[4]);
	memmap_auto_1k(MMPPU(0x1C00), m033.chr[5]);
}
INLINE static void mirroring_fix_033(void) {
	if (m033.prg[0] & 0x40) {
		mirroring_H();
