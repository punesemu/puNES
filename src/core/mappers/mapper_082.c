...
void extcl_cpu_wr_mem_082(WORD address, BYTE value) {
...
void extcl_cpu_every_cycle_082(void) {
...
INLINE static void prg_fix_082(void) {
	memmap_auto_8k(MMCPU(0x8000), prg_reg_control_082(m082.prg[0]));
	memmap_auto_8k(MMCPU(0xA000), prg_reg_control_082(m082.prg[1]));
	memmap_auto_8k(MMCPU(0xC000), prg_reg_control_082(m082.prg[2]));
	memmap_auto_8k(MMCPU(0xE000), 0xFF);
}
INLINE static void chr_fix_082(void) {
	WORD swap = (m082.control & 0x02) << 11;

	memmap_auto_2k(MMPPU(0x0000 ^ swap), (m082.chr[0] >> 1));
	memmap_auto_2k(MMPPU(0x0800 ^ swap), (m082.chr[1] >> 1));
	memmap_auto_1k(MMPPU(0x1000 ^ swap), m082.chr[2]);
	memmap_auto_1k(MMPPU(0x1400 ^ swap), m082.chr[3]);
	memmap_auto_1k(MMPPU(0x1800 ^ swap), m082.chr[4]);
	memmap_auto_1k(MMPPU(0x1C00 ^ swap), m082.chr[5]);
}
INLINE static void wram_fix_082(void) {
	BYTE enable = 0;

	enable = m082.wram_enable[0] == 0xCA;
	memmap_auto_wp_2k(MMCPU(0x6000), 0, enable, enable);
	enable = m082.wram_enable[1] == 0x69;
	memmap_auto_wp_2k(MMCPU(0x6800), 1, enable, enable);
	enable = m082.wram_enable[2] == 0x84;
	memmap_auto_wp_1k(MMCPU(0x7000), 4, enable, enable);
	memmap_disable_1k(MMCPU(0x7400));
	memmap_disable_1k(MMCPU(0x7800));
	memmap_disable_1k(MMCPU(0x7C00));
}
INLINE static void mirroring_fix_082(void) {
