...
void map_init_093(void) {
	...
	map_internal_struct_init((BYTE *)&m093, sizeof(m093));
...
void extcl_cpu_wr_mem_093(BYTE nidx, WORD address, BYTE value) {
	// bus conflict
	m093.reg = value & prgrom_rd(nidx, address);
	prg_fix_093();
	chr_fix_093();
}
...
INLINE static void prg_fix_093(void) {
	memmap_auto_16k(0, MMCPU(0x8000), (m093.reg >> 4));
