...
void prg_swap_jyasic_281(WORD address, DBWORD value);
void chr_swap_jyasic_281(WORD address, DBWORD value);
void wram_swap_jyasic_281(WORD address, DBWORD value);
void mirroring_swap_jyasic_281(WORD address, DBWORD value);

INLINE static DBWORD prg_bank(DBWORD value);
INLINE static DBWORD chr_bank(DBWORD value);

void map_init_281(void) {
	...
	JYASIC_prg_swap = prg_swap_jyasic_281;
	JYASIC_chr_swap = chr_swap_jyasic_281;
	JYASIC_wram_swap = wram_swap_jyasic_281;
	JYASIC_mirroring_swap = mirroring_swap_jyasic_281;
}
...
INLINE static DBWORD prg_bank(DBWORD value) {
	return (((jyasic.mode[3] << 5) | (value & 0x1F)));
}
INLINE static DBWORD chr_bank(DBWORD value) {
	return (((jyasic.mode[3] << 8) | (value & 0xFF)));
}
