...
void map_init_242(void) {
	...
	map_internal_struct_init((BYTE *)&m242, sizeof(m242));
...
void extcl_cpu_wr_mem_242(UNUSED(BYTE nidx), WORD address, UNUSED(BYTE value)) {
	m242.reg = address;
	prg_fix_242();
	chr_fix_242();
	mirroring_fix_242();
}
BYTE extcl_cpu_rd_mem_242(BYTE nidx, WORD address, UNUSED(BYTE openbus)) {
	if (address >= 0x8000) {
		return (m242.reg & 0x100 ? prgrom_rd(nidx, address | dipswitch.value) : prgrom_rd(nidx, address));
	}
