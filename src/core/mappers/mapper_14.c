...
void map_init_14(void) {
	EXTCL_AFTER_MAPPER_INIT(14);
	EXTCL_CPU_WR_MEM(14);
	EXTCL_CPU_RD_MEM(14);
	EXTCL_SAVE_MAPPER(14);
	EXTCL_CPU_EVERY_CYCLE(14);
	EXTCL_PPU_000_TO_34X(14);
	EXTCL_PPU_000_TO_255(14);
	EXTCL_PPU_256_TO_319(14);
	EXTCL_PPU_320_TO_34X(14);
	EXTCL_UPDATE_R2006(14);
	mapper.internal_struct[0] = (BYTE *)&m14;
	mapper.internal_struct_size[0] = sizeof(m14);
...
void extcl_after_mapper_init_14(void) {
	if (m14.reg & 0x02) {
...
void extcl_cpu_wr_mem_14(WORD address, BYTE value) {
	if (address == 0xA131) {
		m14.reg = value;
		extcl_after_mapper_init_14();
		return;
	}
	if (m14.reg & 0x02) {
...
BYTE extcl_cpu_rd_mem_14(WORD address, BYTE openbus, UNUSED(BYTE before)) {
	if (!(m14.reg & 0x02)) {
		return (extcl_cpu_rd_mem_VRC2and4(address, openbus, before));
	}
	return (openbus);
}
BYTE extcl_save_mapper_14(BYTE mode, BYTE slot, FILE *fp) {
	save_slot_ele(mode, slot, m14.reg);
	extcl_save_mapper_MMC3(mode, slot, fp);
	extcl_save_mapper_VRC2and4(mode, slot, fp);

	return (EXIT_OK);
}
void extcl_cpu_every_cycle_14(void) {
	if (m14.reg & 0x02) {
		extcl_cpu_every_cycle_MMC3();
	}
}
void extcl_ppu_000_to_34x_14(void) {
	if (m14.reg & 0x02) {
		extcl_ppu_000_to_34x_MMC3();
	}
}
void extcl_ppu_000_to_255_14(void) {
	if (m14.reg & 0x02) {
		extcl_ppu_000_to_255_MMC3();
	}
}
void extcl_ppu_256_to_319_14(void) {
	if (m14.reg & 0x02) {
		extcl_ppu_256_to_319_MMC3();
	}
}
void extcl_ppu_320_to_34x_14(void) {
	if (m14.reg & 0x02) {
		extcl_ppu_320_to_34x_MMC3();
	}
}
void extcl_update_r2006_14(WORD new_r2006, WORD old_r2006) {
	if (m14.reg & 0x02) {
...
INLINE static WORD chr_base(int slot) {
	switch (slot) {
		...
		case 2:
		case 3:
			return((m14.reg & 0x08) << 5);
		case 4:
		case 5:
			return((m14.reg & 0x20) << 3);
		case 6:
		case 7:
			return((m14.reg & 0x80) << 1);
		default:
			break;
	}
	return (0);
