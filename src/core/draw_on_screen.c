...

		{
			...
			for (y1 = 0; y1 < 8; y1++) {
				...
				for (x1 = 0; x1 < (8 - xl - xr); x1++) {
					...
					if (list[x1] == '@') {
						ppudata.ppu_screen.wr->line[y + y1][x + x1] = color;
					} else if (list[x1] == ',') {
						ppudata.ppu_screen.wr->line[y + y1][x + x1] = 0x0010;
					} else if (list[x1] == '.') {
						ppudata.ppu_screen.wr->line[y + y1][x + x1] = 0x002D;
					} else {
						ppudata.ppu_screen.wr->line[y + y1][x + x1] = background;
					}
				}
				font_y++;
			}
...
void dos_vline(int x, int y, int h, WORD color) {
	...
	for (y1 = 0; y1 < h; y1++) {
		...
		ppudata.ppu_screen.wr->line[y + y1][x] = color;
	}
}
void dos_hline(int x, int y, int w, WORD color) {
	...
	for (x1 = 0; x1 < w; x1++) {
		...
		ppudata.ppu_screen.wr->line[y][x + x1] = color;
	}
}
